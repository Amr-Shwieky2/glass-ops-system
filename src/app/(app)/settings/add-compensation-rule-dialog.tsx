...
import { createCompensationRuleAction, type ActionState } from "@/server/lookups/actions";
...
function SubmitButton() {
  ...
  return (
    <Button type="submit" disabled={pending}>
      {pending ? "جارٍ الحفظ..." : "إضافة قاعدة"}
    </Button>
  );
}

export function AddCompensationRuleDialog({
  workTypes,
}: {
  workTypes: { id: string; labelAr: string }[];
}) {
  const [open, setOpen] = React.useState(false);
  const [workTypeId, setWorkTypeId] = React.useState(NONE);
  const [state, formAction] = useActionState(createCompensationRuleAction, initialState);
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>إضافة قاعدة تعويض جديدة</DialogTitle>
        </DialogHeader>
        <form action={formAction} className="space-y-4" noValidate>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="space-y-2">
              ...
              <Select name="unit" required defaultValue="unit">
